/*
  ...
  - During scanning, line 2 shows "Scanning...".
  - When any grey terrain bin reaches full-scale, scanning stops.
  - After scan complete, line 2 shows the recommended FarmWhisper channel as >Use CH xx.
...
struct SlotStats {
    ...
    // The final recommendation is selected after a terrain bin reaches full-scale.
...
void findBestSlot()
{
    ...
    for (uint8_t i = 0; i < scanSlotCount; i++) {
        ...
        if (!found || slotStats[i].uglyPoints < bestUglyPoints) {
            bestUglyPoints = slotStats[i].uglyPoints;
            chosen = i;
            found = true;
        }
    }
